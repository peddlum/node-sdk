...
import { PeddlumError, PeddlumErrorCode } from './errors';
 ...
 *     envelope into a plain `data` return (throws a typed PeddlumError
 *     on `success: false`).
...
export class HttpClient {
  ...
  private async request<T>(config: AxiosRequestConfig): Promise<T> {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        ...
        lastErr = err;
        const peddlumErr = this.normalizeError(err);
        if (!this.shouldRetry(peddlumErr) || attempt === maxAttempts - 1) {
          throw peddlumErr;
        }
  ...
  private unwrapEnvelope<T>(body: unknown): T {
    if (body && typeof body === 'object' && 'success' in body) {
      ...
      if (env.success === false) {
        const code = (env.error?.code as PeddlumErrorCode) ?? 'UNKNOWN';
        const message = env.error?.message ?? 'Request failed';
        throw new PeddlumError(code, message, { details: env.error });
      }
  ...
  private normalizeError(err: unknown): PeddlumError {
    if (err instanceof PeddlumError) return err;
    if (axios.isAxiosError(err)) {
      ...
      const message =
        ...
        axErr.message ??
        'Request failed';
      return new PeddlumError(code, message, { status, details: envelope });
    }
    if (err instanceof Error) {
      return new PeddlumError('UNKNOWN', err.message);
    }
    return new PeddlumError('UNKNOWN', 'Unknown error', { details: err });
  }
  ...
  private classifyStatus(
    ...
    if (serverCode && this.isValidCode(serverCode)) {
      return serverCode as PeddlumErrorCode;
    }
  ...
  private isValidCode(code: string): boolean {
    // Any string matching the PeddlumErrorCode pattern; we accept anything
    // and let type narrowing surface unexpected codes during development.
    return typeof code === 'string' && code.length > 0;
  }

  private shouldRetry(err: PeddlumError): boolean {
    return (

...
export interface PeddlumClientOptions extends HttpClientOptions {}

/**
 * Entry point for the Peddlum SDK.
 *
...
export class PeddlumClient {
  ...
  constructor(apiKey: string, options: PeddlumClientOptions = {}) {
    if (!apiKey) {
      throw new Error(
        'PeddlumClient: API key is required. Get one from the peddlum seller dashboard.',
      );
    }
    if (!apiKey.startsWith('service_') && !apiKey.startsWith('anon_')) {
      ...
      console.warn(
        'PeddlumClient: API key should start with "service_" or "anon_".',
      );
    }

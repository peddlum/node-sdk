...
const WIDGET_ROOT_ID = 'peddlum-chatbot-widget-root';
/**
...
function loadChatbot(anonKey, options = {}) {
    ...
    if (!anonKey || !anonKey.startsWith('anon_')) {
        ...
        console.warn('[peddlum-chatbot] Prefer anon_ keys in the browser. service_ keys expose full privileges.');
    }
    if (document.getElementById(WIDGET_ROOT_ID))
        return;
    const baseUrl = options.baseUrl ?? constants_1.PEDDLUM_BASE_URL;
...
function buildBubble(primary) {
    ...
    // Inline SVG chat icon (inherits the bubble's white `color` via
    // currentColor) — crisper and more consistent than an emoji, which
    // renders differently across OSes/browsers.
    el.innerHTML =

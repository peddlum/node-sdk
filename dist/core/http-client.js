...
class HttpClient {
    constructor(apiKey, options = {}) {
        ...
        this.axios = axios_1.default.create({
            baseURL: constants_1.PEDDLUM_BASE_URL,
    ...
    unwrapEnvelope(body) {
        if (body && typeof body === 'object' && 'success' in body) {
            ...
            if (env.success === false) {
                ...
                throw new errors_1.PeddlumError(code, message, { details: env.error });
            }
            return env.data;
        }
        return body;
    ...
    normalizeError(err) {
        if (err instanceof errors_1.PeddlumError)
            return err;
        if (axios_1.default.isAxiosError(err)) {
            ...
            const message = envelope?.error?.message ??
                axErr.message ??
                'Request failed';
            return new errors_1.PeddlumError(code, message, { status, details: envelope });
        }
        if (err instanceof Error) {
            return new errors_1.PeddlumError('UNKNOWN', err.message);
        }
        return new errors_1.PeddlumError('UNKNOWN', 'Unknown error', { details: err });
    }
